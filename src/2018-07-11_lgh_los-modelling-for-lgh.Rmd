---
title: "LOS modelling for Lions Gate Hospital"
author: "Nayef Ahmad - VCH Decision Support"
date: "July 11, 2018"
output:
  html_document:
      toc: yes

---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
options(knitr.table.format = "html")

library("tidyverse")
library("magrittr")
library("kableExtra")
library("here")
library("broom")
library("ggpubr")
library("ggplot2")
library("readr")
library("VGAM")

# shortcuts: 
# > ctrl + alt + i ==> new code chunk
# > ctrl + shift + k ==> knit doc 
# \  ==> line break (include 2 spaces after)
# 


# rm(list = ls())
# load models: 
if(!exists("m4.los.vs.dow.age.unit")){
    source(here("src", 
                "2018-07-09_lgh_los-by-dow_zero-truncated-poisson-regression.R"))
}

if(!exists("p17.test.data.actual.vs.pred.m0.unit")){
    source(here("src", 
                "2018-07-10_lgh_testing-los-model-on-test-data.R"))
}


# load dummy data: 
df6.dummy.age <- read_csv(here("results", 
                               "wip", 
                               "2018-07-12_rgnl_los-dummy-data-effect-of-age.csv"))
df6.dummy.age %<>% mutate_if(is.character, factor)


df7.dummy.dow <- read_csv(here("results", 
                               "wip", 
                               "2018-07-12_rgnl_los-dummy-data-effect-of-dow.csv"))
df7.dummy.dow %<>% 
    mutate_if(is.character, factor)%>% 
    filter(!is.na(losdays))


```

## Summary
Preliminary data analysis shows that Length of stay (LOS) at Lions Gate Hospital may depend on the __day of the week of admission__. To test this hypothesis, we must isolate the effect of the day of week of admission by adjusting for other variables such as __patient age__ and __admission nursing unit__. 

To quantify these relationship, we developed a multivariate model to predict average LOS using patient age, admission unit and admission day of week. Using all 3 of these variables improves prediction by around 4.4% compared to the baseline model that only uses nursing unit to predict average LOS (using mean absolute error for comparison). Thus, even though the model was built primarily for inference, it may be useful for prediction as well. The mean absolute error of predictions from the model is 3.1 days. 

We conclude that there is an effect on LOS due to day of week of admission. Using Monday as a baseline: 

* patients admitted on __Thursdays__ have a significantly higher LOS by `r round((exp(0.0528716) - 1)*100, 2)`% on average. 
* On __Tuesdays, Wednesdays, and Saturdays__, average LOS is significantly lower by `r round((1- exp(-0.1137544)) *100, 2)`%, `r round((1-exp(-0.0895939))*100, 2)`% and `r round((1 - exp(-0.1633314))*100, 2)`%. 
* LOS on __Fridays and Sundays__ is not significantly different from Mondays.  

The multivariate regression model we developed has the general form: 

$${log(avgLOS)_i} = \alpha + \beta_{1} nurse.unit_{i} + \beta_{2}  age_{i} + \beta_{3}  day.of.week_{i} + \epsilon_{i}$$
The coefficients for each level of the variables in the model are included in the appendix. 


Examples of other questions that arise from the interaction of the effects of age, nursing unit and day of week include: Is the effect of age different for patients admitted to different units, after adjusting for day of week? Is the effect of weekends different in surgery units and medical units after adjusting for age? These questions are important because managers make decisions based on their intuitive understanding of these relationshipS. In LGH, a new position was recently created ("dishcarge chaser") partly based on the belief that patients admitted around the weekend have longer LOS.  


------------------------------------------------

## Illustrative Examples 
The tables below illustrate how the model adjusts the predicted average LOS based on patient age and day of week. In the first table, only the age is varied for patients admitted on Monday to 4E and 6E. The first six rows show predictions for 4E, the next six show predictions for 6E. 

In the second table, only day of week is varied for patients of age 40 admitted to 4E and 6E. 

#### Effect of age (adjusted for other variables): 
```{r echo=FALSE} 
pred <- predict(m4.los.vs.dow.age.unit, 
                newdata = df6.dummy.age, 
                type = "response")

df6.dummy.age %>% 
    mutate(los.predicted = pred) %>% 
    select(site, 
           dow, 
           age, 
           unit.code, 
           los.predicted) %>% 
    kable() %>% 
    kable_styling(bootstrap_options = c("striped", "condensed"),
                  full_width = FALSE, 
                  position = "left")

```

-----------------------------------------------
\ 
\  
\  

#### Effect of day of week (adjusted for other variables): 
```{r echo=FALSE}
pred <- predict(m4.los.vs.dow.age.unit, 
                newdata = df7.dummy.dow, 
                type = "response")

df7.dummy.dow %>% 
    mutate(los.predicted = pred) %>% 
    select(site, 
           dow, 
           age, 
           unit.code, 
           los.predicted) %>% 
    kable() %>% 
    kable_styling(bootstrap_options = c("striped", "condensed"),
                  full_width = FALSE, 
                  position = "left")


```

---------------------------------------------------

## Methodology
LOS data is strictly positive and highly non-normal, which is why ordinary least squares regression is not appropriate. We used a zero-truncated Poisson regression model (for details, see [this example](https://stats.idre.ucla.edu/r/dae/zero-truncated-poisson/)). The model was trained on data from the full calendar year of 2017, and tested on data from January and February 2018. 

Likelihood ratio tests were used to test for significant improvements in model performance as variables were added to the model. Model residuals were inspected visually and using quantile regression to assess the assumption that the response variable has a zero-truncated Poisson distribution. 

The graphs below illustrate the model performance on the training and test data.

\  

```{r}
p14.actual.vs.pred.m4
```

\  
\  

```{r}
p15.test.data.actual.vs.pred.m4
```



---------------------------

\  
\  
\  

# Possible extensions to the model
It may be worth extending this model both as a tool for inference about the relationships between variables that affect LOS, and for the purpose of predicting LOS at Lions Gate Hospital. The following variables may be useful in improving the predictions of the model: 

* Gender
* Admission Category Group              
* Admission Patient Service (current encounter and previous encounters)
* Number of previous acute episodes in last year 
* ED admit vs direct admit 
* Chronic diseases 


------------------------------
\  
\  
\  
\  
\  

# Appendix: Zero-truncated Poisson Regression Model
```{r}
summary(m4.los.vs.dow.age.unit)
```



\  
\  
\  

### Comparison with null model 
```{r}
df5.error.comparison %>% 
    kable() %>% 
    kable_styling(bootstrap_options = c("striped", "condensed"),
                  full_width = FALSE, 
                  position = "left")
```




